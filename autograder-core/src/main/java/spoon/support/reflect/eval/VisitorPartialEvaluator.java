...
public class VisitorPartialEvaluator extends CtScanner implements PartialEvaluator {
    ...
    /**
     * Sets the result of the evaluation to the given element.
     *
     * @param element the element to set as a result
     */
    protected void setResult(CtElement element) {
        result = element;
    }

    /**
     * Gets the current result of the evaluation.
     *
     * @return the result of the evaluation, which might be null
     */
    ...
    public <T> void visitCtBinaryOperator(CtBinaryOperator<T> operator) {
        ...
        if ((left instanceof CtLiteral) && (right instanceof CtLiteral)) {
            ...
            // the unbox ensures that the type of an integer-literal is not Integer but int
            res.setType(operator.getFactory().Type().createReference(res.getValue().getClass()).unbox());
            setResult(res);
            ...
            switch (operator.getKind()) {
                case AND:
                    if ((Boolean) o) {
                        ...
                        res.setValue(false);
                        res.setType(operator.getFactory().Type().BOOLEAN_PRIMITIVE);
                        setResult(res);
                    }
                    return;
                case OR:
    ...
    private <T> void visitFieldAccess(CtFieldAccess<T> fieldAccess) {
        // convert a Type.class access to a literal of the class
    ...
    private static class CharacterNumber extends Number {
        private final char value;

        CharacterNumber(char value) {
            this.value = value;
        }
    ...
    private static Number toNumber(Object object) {
        if (object instanceof Character) {
            return new CharacterNumber((Character) object);
        } else {
            return (Number) object;
        }
    }

    private static Object fromNumber(Number number) {
        if (number instanceof CharacterNumber) {
            return ((CharacterNumber) number).value;
        } else {
            return number;
        }
    }
    ...
    public <T> void visitCtUnaryOperator(CtUnaryOperator<T> operator) {
        ...
        if (operand instanceof CtLiteral) {
            ...
            switch (operator.getKind()) {
                case POS:
                    // TODO: this might be broken, test with all supported types
                    value = +toNumber(object).intValue();
                    break;
                case NEG: {
                    // NOTE: type can not be boolean, so this conversion is safe:
                    Number number = toNumber(object);

                    if (isFloatingType(operator.getType())) {
                        value = convert(operator.getType(), -1 * number.doubleValue());
                    } else {
                        value = convert(operator.getType(), -1 * number.longValue());
                    }
                    break;
                }
                case NOT:
                ...
                case COMPL:
                    // NOTE: type can not be boolean, so this conversion is safe:
                    value = convert(operator.getType(), ~toNumber(object).longValue());
                    break;
                case PREINC:
                case PREDEC:
                case POSTINC:
                case POSTDEC: // those operators can not be applied to literals
                    throw new RuntimeException("unsupported operator " + operator.getKind());
                default:
                    throw new RuntimeException("unsupported operator " + operator.getKind());
            }

            // convert it back to the original type (so that a char is a char again)
            if (value instanceof Number) {
                value = fromNumber((Number) value);
            }
